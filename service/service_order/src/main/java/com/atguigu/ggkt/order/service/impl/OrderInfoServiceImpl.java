...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo> implements OrderInfoService {
    ...
    public Long submitOrder(OrderFormVo orderFormVo) {
        ...
        if (orderDetailExists != null) {
            return orderDetailExists.getId();//订单已成生产,直接返回id
        }
        ...
        //6.1封装数据到OrderInfo对象里面，添加订单基本信息
        ...
        //6.2封装数据到OrderDetail对象里面，添加订单基本信息
        ...
        if(null != orderFormVo.getCouponUseId()){
            couponInfoFeignClient.updateCouponInfoUserStatus(orderFormVo.getCouponUseId(),orderInfo.getId());
        }
    ...
    public Map<String, Object>  selectOrderInfoPage(Page<OrderInfo> pageParam,
        ...
        if(!StringUtils.isEmpty(createTimeBegin)){
            wrapper.ge("create_time_begin", createTimeBegin);
        }
        if(!StringUtils.isEmpty(createTimeEnd)){
            wrapper.le("create_time_end", createTimeEnd);
        }
